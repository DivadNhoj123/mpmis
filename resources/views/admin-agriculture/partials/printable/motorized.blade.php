...
<head>
    ...
    <style>
        ...
        body {
            ...
            color: #000;
            font-size: 14px;
            /* Set the default font size to 12px */
        }
...
<body
    onload="window.print(); setTimeout(() => window.location.href = '{{ route('agriculture-motorized.index') }}', 1000);">
    ...
    <h4 class="text-center">{{ date('Y') }} MASTERLIST OF MOTORIZED FISHING BOAT LICENSES & REGISTRATION</h4>
        ...
        <tbody>
            @foreach ($register as $register)
                <tr>
                    ...
                    <td>{{ $register->engine_serial }}</td>
                    <td>{{ $register->hp }}</td>
                    <td>{{ $register->color }}</td>
                    <td>{{ $register->tonnage }}</td>
                    <td>
                        @php
